...
For naming decisions, follow these preferences, adapted from Alex Edwards' Go naming conventions:
...
- Match name length to scope: short names suit tight local scopes; wider scopes need more descriptive names.
- Avoid type-encoding names except when distinguishing representations during conversion, such as `userID` and
  `userIDStr`.
- Avoid shadowing builtins and imported package names.
- Use short lowercase package names, preferably one clear word. Avoid catch-all names such as `common`, `util`,
...
Prefer a deliberate project convention or external naming contract over mechanical renaming.
